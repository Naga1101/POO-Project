...
public class VintageMain implements Serializable
{
    ...
    public void loadArtigosPublicados(){
        for(User user : listaUsersRegistados){
            List<Tuple<Artigos, String>> listaVendas = user.getListaVendas();
            for (Tuple<Artigos, String> elem : listaVendas) {
                artigosPublicados.add(elem.getFirst());
            }
        }
    }
    ...
    public void realizarEncomenda(ArrayList<Artigos> listaArtigos) {    
        for (Artigos artigo : listaArtigos) {
            for (User user : listaUsersRegistados) {
                ArrayList<Tuple<Artigos, String>> listaVendas = user.getListaVendas();
                for (Tuple<Artigos, String> elem : listaVendas) {
                    Artigos artigoRem = elem.getFirst();
                    if (artigoRem.equals(artigo)) {
                        user.removerArtigoVender(artigoRem.getCodigo());
                        break;
                    }
                }
            }
    ...
    public void encomendaChegou(ArrayList<Artigos> listaArtigos) {    
        for (Artigos artigo : listaArtigos) {
            for (User user : listaUsersRegistados) {
                ArrayList<Tuple<Artigos, String>> listaVendas = user.getListaVendas();
                for (Tuple<Artigos, String> elem : listaVendas) {
                    Artigos artigoRem = elem.getFirst();
                    if (artigoRem.equals(artigo)) {
                        user.encomendaEntregue(artigoRem.getCodigo());
                    }
                }
            }
        }
        ...
        for (User user : listaUsersRegistados) {
                ...
                for (Encomenda encomenda : encomendas) {
                    ...
                    if (entrega.isBefore(dataAtual)) {
                        encomendaChegou(encomenda.getArtigos());
                    }
                }
        }
    }
    ...
    public void pagarVendedor(ArrayList<Artigos> listaArtigos){
        for (Artigos artigo : listaArtigos) {
            for (User user : listaUsersRegistados) {
                ...
                for (Tuple<Artigos, String> elem : listaVendas) {
                    Artigos artigoRem = elem.getFirst();
                    System.out.println(artigoRem.toString());
                    if (artigoRem.equals(artigo)) {
    ...
    public void removerAnuncio(ArrayList<Artigos> listaEncomendada) {
        for(Artigos artigo : listaEncomendada){
            Artigos elem = procurarArtigo(artigo.getCodigo());
            artigosPublicados.remove(elem);
        }
    }
    ...
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        while (!respostaValida) {
            ...
            if (aux == 'S') {
                ...
                ArrayList<Tuple<Artigos, String>> vaziaV = new ArrayList<>();
                ArrayList<Encomenda> vaziaE = new ArrayList<>();
                User user1 = new User(21,"joao","João","Rua Banana",100000001, 0,vazia,vaziaV, vaziaE);
                User user2 = new User(20,"maria","Maria","Rua Melancia",100024001, 0,vazia, vaziaV, vaziaE);
                User user3 = new User(35,"zubyo","Zubyo","Rua Melão",113024001, 0,vazia, vaziaV, vaziaE);
                User user4 = new User(28,"zulnae","Zulnae","Rua Abóbora",100928301, 0,vazia, vaziaV, vaziaE);
                User user5 = new User(28,"gaia","Gaia","Rua Maça",390928301, 0,vazia, vaziaV, vaziaE);
                Transportadoras transportadora1 = new Transportadoras(15, "CTT", false, 7, 0.5, 0);
                Transportadoras transportadora2 = new Transportadoras(18, "vip", true, 10, 2, 0);
